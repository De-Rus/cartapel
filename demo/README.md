...
A self-contained example so cartapel has something to show on first run — and a
...
- [`admin/`](admin/) — the config directory (`--config`):
  - `config/` — the reserved globals: `cartapel.hcl` (brand + the `main`
...
toggles in `config/cartapel.hcl` disable the only capabilities that would make an
...
These are generic cartapel options — useful on any sensitive instance, not just a
...
too. Point cartapel at a **throwaway database** for a public demo regardless.
