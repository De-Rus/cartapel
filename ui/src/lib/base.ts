...
//
// The server injects the live `--base-path` / `CARTAPEL_BASE_PATH` into the page
// as `window.__CARTAPEL_BASE__` (see index.html + the server's SPA handler), so a
...
declare global {
  interface Window {
    __CARTAPEL_BASE__?: string
  }
}

const raw = typeof window !== 'undefined' ? window.__CARTAPEL_BASE__ : undefined
