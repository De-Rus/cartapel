...
- `--db` — the database connection URL (`postgres://…` or `mysql://…` — the scheme picks the engine; ClickHouse attaches as a secondary read-only [source](/configuration/sources)). Also settable via `CARTAPEL_DB`, or as
